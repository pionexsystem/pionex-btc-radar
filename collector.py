#!/usr/bin/env python3
"""Pionex BTC Radar Data Collector v0.1.
...
SESSION.headers.update({
    "User-Agent": "pionex-btc-radar/0.1",
...
def classify(h4: dict[str, Any], d1: dict[str, Any]) -> dict[str, Any]:
    score = h4["evidence"]["score"] + d1["evidence"]["score"]
    direction = "BULL" if score >= 2 else "BEAR" if score <= -2 else "SIDEWAYS"
    sign = 1 if direction == "BULL" else -1 if direction == "BEAR" else 0
    tf4, tf1 = h4["evidence"]["timeframe_direction"], d1["evidence"]["timeframe_direction"]
    divergence = tf4 in {"BULL", "BEAR"} and tf1 in {"BULL", "BEAR"} and tf4 != tf1
    character = "CHAOS" if divergence else "TREND" if direction != "SIDEWAYS" and h4["adx"] >= 25 and d1["adx"] >= 20 else "CONSOLIDATION"
    checks = {
        ...
        "timeframe_alignment": tf4 == tf1 == direction,
    }
    count = sum(checks.values())
    strength = "WEAK" if count <= 1 else "MEDIUM" if count == 2 else "STRONG"
    if divergence and strength == "STRONG":
        strength = "MEDIUM"
    return {"direction": direction, "character": character, "signal_strength": strength, "confirmation_count": count, "confirmations": checks, "timeframe_direction_4h": tf4, "timeframe_direction_1d": tf1, "timeframe_divergence": divergence, "raw_direction_score": score, "method_id": METHOD_ID, "method_version": METHOD_VERSION}
...
def main() -> int:
    ...
    run_id = f"run-btc-radar-{local.strftime('%Y%m%dT%H%M')}{local.strftime('%Z')}"
    ...
    if bundle is None:
        latest = {"schema_version": "btc-radar-input-0.1.0", "run_id": run_id, "status": "FAILED", "generated_at": local.isoformat(), "source": None, "indicators": {"4h": None, "1d": None}, "classification": None, "baseline": {"status": "NOT_UPDATED"}, "quality_gate": "FAIL", "information_risk": "HIGH", "errors": errors, "safety": {"read_only": True, "execution_allowed": False, "human_gate_required": True}}
        atomic_json(LATEST, latest)
    else:
        try:
            h4, d1 = indicators(bundle["4h"]), indicators(bundle["1d"])
            classification = classify(h4, d1)
            raw_pack = {"run_id": run_id, "generated_at": local.isoformat(), "source": {k: bundle[k] for k in ("venue", "role", "qualification", "hostname", "instrument", "market")}, "ticker": bundle["ticker"], "candles_4h": bundle["4h"].to_dict("records"), "candles_1d": bundle["1d"].to_dict("records"), "acquisition_meta": bundle["meta"]}
            raw_hash = sha256(raw_pack)
            raw_path = RAW / f"{run_id}.json"
            atomic_json(raw_path, raw_pack)
            snapshot = {"run_id": run_id, "source": bundle["venue"], "instrument": bundle["instrument"], "market": bundle["market"], "method_id": METHOD_ID, "method_version": METHOD_VERSION, "ticker_price": bundle["ticker"]["price"], "4h": h4, "1d": d1, "classification": classification}
            baseline_path = BASELINES / f"{bundle['venue'].lower()}.json"
            previous = load_json(baseline_path, None)
            comparable = bool(previous and all(previous.get(k) == snapshot.get(k) for k in ("source", "instrument", "market", "method_id", "method_version")))
            baseline = {"status": "COMPARABLE" if comparable else "NO_COMPARABLE_BASELINE", "baseline_run_id": previous.get("run_id") if comparable else None}
            if comparable:
                baseline["changes"] = {"ticker_price_delta_pct": (snapshot["ticker_price"] / previous["ticker_price"] - 1) * 100, "classification_previous": previous["classification"], "classification_current": classification, "4h_atr_pct_delta": h4["atr_pct"] - previous["4h"]["atr_pct"], "1d_atr_pct_delta": d1["atr_pct"] - previous["1d"]["atr_pct"]}
            control = controls(bundle["ticker"]["price"])
            major = any(x.get("comparison") == "MAJOR_SOURCE_CONFLICT" for x in control.values())
            latest = {"schema_version": "btc-radar-input-0.1.0", "run_id": run_id, "status": "COMPLETE", "generated_at": local.isoformat(), "timezone": TZ_NAME, "method": {"id": METHOD_ID, "version": METHOD_VERSION, "parameters": {"ema": [50, 200], "dmi_period": 14, "adx_smoothing": 6, "atr_period": 14, "maobv_period": 30, "completed_candles_only": True, "requested_candles": CANDLE_LIMIT}}, "source": {"venue": bundle["venue"], "role": bundle["role"], "qualification_status": bundle["qualification"], "hostname": bundle["hostname"], "instrument": bundle["instrument"], "market": bundle["market"], "availability": "A"}, "ticker": bundle["ticker"], "indicators": {"4h": h4, "1d": d1}, "classification": classification, "controls": control, "baseline": baseline, "raw_pack": {"path": str(raw_path.relative_to(ROOT)).replace('\\', '/'), "sha256": raw_hash}, "quality_gate": "FAIL" if major else "WARN", "information_risk": "HIGH" if major else "MEDIUM", "errors": errors, "safety": {"read_only": True, "execution_allowed": False, "human_gate_required": True}}
            atomic_json(LATEST, latest)
            atomic_json(baseline_path, snapshot)
        except Exception as exc:
            errors.append({"source": bundle.get("venue"), "error_class": "CALCULATION_FAILURE", "stage": "CALCULATION_OR_WRITE", "message": f"{type(exc).__name__}: {exc}", "details": {}})
            latest = {"schema_version": "btc-radar-input-0.1.0", "run_id": run_id, "status": "FAILED", "generated_at": local.isoformat(), "source": {"venue": bundle.get("venue")}, "indicators": {"4h": None, "1d": None}, "classification": None, "baseline": {"status": "NOT_UPDATED"}, "quality_gate": "FAIL", "information_risk": "HIGH", "errors": errors, "safety": {"read_only": True, "execution_allowed": False, "human_gate_required": True}}
            atomic_json(LATEST, latest)

    log = load_json(RUN_LOG, [])
    log = log if isinstance(log, list) else []
    log.append({"run_id": latest["run_id"], "generated_at": latest["generated_at"], "status": latest["status"], "source": (latest.get("source") or {}).get("venue"), "direction": (latest.get("classification") or {}).get("direction"), "character": (latest.get("classification") or {}).get("character"), "signal_strength": (latest.get("classification") or {}).get("signal_strength"), "quality_gate": latest["quality_gate"], "information_risk": latest["information_risk"]})
    ...
    print(json.dumps({"run_id": latest["run_id"], "status": latest["status"], "source": (latest.get("source") or {}).get("venue"), "classification": latest.get("classification"), "quality_gate": latest["quality_gate"], "information_risk": latest["information_risk"]}, ensure_ascii=False, indent=2))
